...
export default function TherapyPage() {
  ...
  const sendToApi = async (text: string, overrideSessionId?: string) => {
    setIsTyping(true);
    const activeSessionId = overrideSessionId || sessionId;
    try {
      if (!activeSessionId) throw new Error("No session ID");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await sendToApi(currentMessage);
  };
  ...
  const handleDeleteSession = async (e: React.MouseEvent, sessionToDelete: string) => {
    e.stopPropagation();
    ...
    try {
      ...
      // If we deleted the currently active session, move to a new blank session
  ...
  const handleSuggestedQuestion = async (text: string) => {
    ...
    if (!currentSessionId || currentSessionId === "new") {
      try {
        ...
        console.error("Error creating session for suggested question:", error);
        return;
      }
    }
    ...
    await sendToApi(text, currentSessionId);
  };
      ...
      <div className="flex h-[calc(100vh-4rem)] mt-20 gap-6">
        ...
        <div className="flex-1 flex flex-col overflow-hidden bg-white dark:bg-background rounded-lg border">
          ...
          {messages.length === 0 ? (
                ...
                <div className="grid gap-3 relative">
                  ...
                  {SUGGESTED_QUESTIONS.map((q, index) => (
                    ...
                    >
                      <Button
                        variant="outline"
                        className="w-full h-auto py-4 px-6 text-left justify-start hover:bg-primary/5 hover:border-primary/50 hover:text-primary transition-all duration-300 cursor-pointer active:scale-[0.98]"
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 bg-muted/10 relative overflow-y-auto w-full">
              <Button 
                variant="ghost" 
                size="icon" 
                ...
                onClick={() => {
                  ...
                  const msg = stressPrompt.originalMessage;
              ...
              <div className="text-center mb-8">
                <h2 className="text-2xl font-semibold mb-2">{stressPrompt.activity.title}</h2>
                <p className="text-muted-foreground">{stressPrompt.activity.description}</p>
              </div>
              <div className="w-full max-w-2xl bg-background rounded-xl shadow-sm border p-8 h-[450px]">
                ...
                <AnimatePresence initial={false}>
                  ...
                  {stressPrompt && !showActivity && (
                    <motion.div
                      initial={{ opacity: 0, y: 20 }}
                      animate={{ opacity: 1, y: 0 }}
                      className="px-6 py-8 bg-muted/30"
                    >
                      <div className="flex gap-4">
                        <div className="w-8 h-8 shrink-0 mt-1">
                          <div className="w-8 h-8 rounded-full bg-primary/10 text-primary flex items-center justify-center ring-1 ring-primary/20">
                        ...
                        <div className="flex-1 space-y-4">
                          <div>
                            ...
                            <p className="text-sm mt-2">
                              I notice you mentioned feeling {stressPrompt.trigger}. Would you like to try a quick calming activity?
                            </p>
                          </div>
                          <div className="flex gap-3">
                            ...
                            >
                              Try {stressPrompt.activity.title}
                            </Button>
                            <Button 
                              variant="outline" 
                              onClick={() => {
