...
// Removed unused `calculateDailyStats`
...
export default function Dashboard() {
  ...
  const loadActivities = useCallback(async () => {
    try {
      ...
      // Transform backend Mood data to frontend Activity format
      const moodEntries = moodData.data || [];
      const moodActivities: Activity[] = moodEntries.map((m: any) => ({
        id: m._id,
        userId: m.userId,
        type: "mood",
        name: "Mood Check-in",
      ...
      // Transform backend Activity data to frontend Activity format
      const loggedActivities: Activity[] = (activityData.data || []).map((a: any) => ({
        id: a._id,
        userId: a.userId,
        type: a.type,
        name: a.name,
      ...
      const combinedActivities = [...moodActivities, ...loggedActivities].sort(
        (a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()
      );

      setActivities(combinedActivities);
      setActivityHistory(transformActivitiesToDayActivity(combinedActivities));
    } catch (error) {
  ...
  // Update insights when activities change
  useEffect(() => {
  ...
  const fetchDailyStats = useCallback(async () => {
    try {
      ...
      // Calculate mood score from existing loaded activities for TODAY
      ...
      setDailyStats({
        ...
        completionRate: 100,
        mindfulnessCount: sessions.length, // Total number of therapy sessions from DB
